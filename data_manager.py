...
class DataManager(object):
    ...
    def __init__(self):
        ...
        # X = data[['x1', 'x2', 'x3']].values  # IndexError: invalid index to scalar variable.
        # Y = data[['y0', 'y1']].values 
        # X = np.vstack(X)
        # Y = np.vstack(Y)
        ...
        # self.linear_velocities  = X[:,1] # (49999,)
        # self.angular_velocities = X[:,0] # (49999,)
        # self.pos_xs = Y[:,0]         # (49999,) -4.5~4.5
        # self.pos_zs = Y[:,1]         # (49999,) -4.5~4.5
        # self.angles = X[:,0] * np.pi # (49999,) -pi~pi
    def prepare(self, place_cells, hd_cells):
        ...
        self.inputs[:,2] = np.cos(self.angular_velocities)
        self.inputs[:,3] = np.sin(self.angular_velocities)
